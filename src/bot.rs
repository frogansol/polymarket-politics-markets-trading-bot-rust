...
impl Bot {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        logger::divider();
        logger::log("CONFIG", "Polymarket Politics Markets Bot (Rust)");
        logger::divider();
        ...
        logger::log("CONFIG", &format!("Politics keywords: {}", self.config.politics_keywords.join(", ")));
        logger::divider();
    ...
    async fn discover_and_enter_markets(&mut self) -> Result<()> {
        let markets = gamma::find_politics_markets(
            &self.http,
            &self.config.gamma_host,
            &self.config.politics_keywords,
        )
        .await?;
        let live = self.filter_live_markets(markets);
        if live.is_empty() {
            logger::warn("LOOP", "No active politics markets matching keywords – will retry");
            return Ok(());
        }
